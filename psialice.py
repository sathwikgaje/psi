...
async def main():
    ...
    p = prover()
    param = v.setup()
    aliceMacKey = key_gen(param)
    bobSecretKey, bobPublicKey = make_keypair()
    aliceCommitment, aliceR = p.commit(param, aliceMacKey)
    temp = [aliceCommitment,aliceR,bobPublicKey]
    co_eff = await mpc.transfer(temp)
    temp1 = co_eff[other_party]
    bobCommitment = temp1[0]
    bobR = temp1[1]
    alicePublicKey = temp1[2]
    transfer_set = []
    ts = time.time()
    for i in set:
        transfer_set.append((aliceMacKey*i)-(scalar_mult(i,alicePublicKey)[0])+(scalar_mult(i,bobPublicKey)[0]))
    computation_time = computation_time + (time.time() - ts)
    ti = await mpc.transfer(time.time())
    sets = await mpc.transfer(transfer_set)
    ma = await mpc.transfer(aliceMacKey)
    bobMacKey = ma[other_party]
    commucation_time = commucation_time + (time.time() - ti[0])
    result1 = v.open(param, bobCommitment, bobMacKey, bobR)
    if result1 == False:
    ...
    PSI = []
    aliceSet = sets[party]
    bobSet = sets[other_party]
    for i in range(len(aliceSet)):
        for j in range(len(bobSet)):
            if aliceSet[i]+bobSet[j] == (aliceMacKey+bobMacKey)*set[i]:
                PSI.append(set[i])
    computation_time = computation_time + (time.time() - ts)
    print(PSI)
